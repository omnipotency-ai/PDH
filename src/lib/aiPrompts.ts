...
import { DR_POO_MODEL, getModelLabel } from "@/lib/aiModels";
...
export function sanitizeProfileField(value: string, maxLen: number): string {
  const withoutBidi = value.replace(/[\u200E\u200F\u202A-\u202E\u2066-\u2069]/g, "");
...
export function getAiDisclaimer(model: string = DR_POO_MODEL): string {
...
/**
 * Compute the food log time window in hours based on surgery type.
 *
 * Base window:
...
export function getFoodWindowHours(profile: HealthProfile): number {
  return Math.min(profile.surgeryType === "Ileostomy reversal" ? 48 : 72, MAX_FOOD_WINDOW_HOURS);
}
 ...
 * - BM logs: 48h always
 * - Habits, fluids, activities, and all other logs: 24h always
 */
 ...
 */
export function computeBristolTrend(weeklyDigests: WeeklyContext[]): string {
...
const FAMILIARITY_DIRECTIVES: Record<ToneFamiliarity, string> = {
  reserved:
    "Keep the tone detached, plain, and data-led. Avoid cheerleading and keep emotional commentary to a minimum.",
  steady:
    "Keep the tone calm, respectful, and grounded. Sound like a clinician who is clear without being cold.",
  familiar:
    "Keep the tone human, direct, and conversational. Sound like someone who knows the patient's history well.",
  close:
    "Keep the tone warm, candid, and supportive. Be encouraging without becoming fluffy or theatrical.",
};

const VOCABULARY_DIRECTIVES: Record<ToneVocabulary, string> = {
  everyday:
    "Use everyday language and avoid jargon unless a medical term is genuinely necessary.",
  balanced:
    "Mix plain English with useful clinical labels, briefly translating technical terms when they help.",
  clinical:
    "Use clinical terminology confidently, but keep it readable and explain complex terms in plain English.",
};

const OUTPUT_STYLE_DIRECTIVES: Record<OutputStyle, string> = {
  prose: "Default to short paragraphs. Use bullets only when the content is genuinely list-shaped.",
  blended: "Use a mix of short paragraphs and compact bullets. Structure clearly without feeling rigid.",
  structured: "Default to compact, list-led structure with strong signposting and minimal filler prose.",
};

const OUTPUT_LENGTH_DIRECTIVES: Record<OutputLength, string> = {
  brief: "Keep the default response lean: essentials first, minimal elaboration unless the user asks.",
  standard: "Give balanced detail: enough reasoning to be useful without turning every reply into an essay.",
  detailed:
    "Give fuller reasoning and context by default, while still prioritising clarity over repetition.",
};

function buildToneDirective(prefs: AiPreferences): string {
  return [
    FAMILIARITY_DIRECTIVES[prefs.toneFamiliarity],
    VOCABULARY_DIRECTIVES[prefs.toneVocabulary],
  ].join(" ");
}

function buildOutputDefaultsDirective(prefs: AiPreferences): string {
  return [
    OUTPUT_STYLE_DIRECTIVES[prefs.outputStyle],
    OUTPUT_LENGTH_DIRECTIVES[prefs.outputLength],
  ].join(" ");
}
...
export function buildSystemPrompt(profile: HealthProfile, prefs: AiPreferences): string {
  const surgeryLabel =
    profile.surgeryType === "Other" && profile.surgeryTypeOther
      ? profile.surgeryTypeOther
      : profile.surgeryType;
  ...
  const clinicalHistoryValue = sanitizeProfileField(profile.clinicalHistory ?? "", 3000);
  const clinicalHistoryBlock = clinicalHistoryValue
    ? `- Clinical history:\n${clinicalHistoryValue
        .split("\n")
        .map((line) => `  ${line}`)
        .join("\n")}`
    : "";
...
${buildToneDirective(prefs)}

Default output behaviour: ${buildOutputDefaultsDirective(prefs)}
...
- Honour the user's style settings first: outputLength and outputStyle are the default for detail and structure.
