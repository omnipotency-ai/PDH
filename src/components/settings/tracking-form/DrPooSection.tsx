...
import type { AiPreferences, DrPooPreset } from "@/types/domain";
...
export function DrPooSection({ aiPreferences, setAiPreferences }: DrPooSectionProps) {
  ...
  return (
    <div data-slot="dr-poo-section" className="space-y-2">
      ...
      <div className="space-y-2 rounded-xl border border-sky-400/25 bg-[var(--surface-2)] p-3">
        <div className="space-y-1">
          <Label className="text-xs justify-center text-[var(--text)]/70">
            Dr. Poo tone and output
          </Label>
          <p className="text-[10px] text-[var(--text-faint)]">
            Pick a preset, or fine-tune the familiarity, vocabulary, structure, and detail
            directly.
          </p>
        </div>
          ...
          {PRESET_CARDS.map((preset) => {
            ...
            return (
              <div
                key={preset.value}
                className={cn(
                  "rounded-lg border bg-[var(--surface-0)] px-3 py-2 hover:bg-[var(--surface-1)]",
                  isSelected ? "border-sky-400/80" : "border-sky-400/25",
                )}
              >
                <div className="flex items-start gap-2">
                  <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-full bg-[var(--surface-2)]">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <SliderControl
            label="Tone familiarity"
            value={aiPreferences.toneFamiliarity}
            options={FAMILIARITY_OPTIONS}
            onChange={(value) => setAxisValue({ toneFamiliarity: value })}
          />

          <SliderControl
            label="Tone vocabulary"
            value={aiPreferences.toneVocabulary}
            options={VOCABULARY_OPTIONS}
            onChange={(value) => setAxisValue({ toneVocabulary: value })}
          />

          <SliderControl
            label="Output style"
            value={aiPreferences.outputStyle}
            options={STYLE_OPTIONS}
            onChange={(value) => setAxisValue({ outputStyle: value })}
          />
