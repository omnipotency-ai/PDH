...
**Updated:** 2026-04-04
**MINOR — Fixed:** 15/25 | **Open:** 10/25
**NICE-TO-HAVE — Fixed:** 9/23 | **Open:** 14/23
**Overall (all 84 findings): Fixed 55 | Open 29**
...
### 37. [FIXED] `useEffect` escape-key handler re-attaches on every view transition
...
### 38. [OPEN] Camera/Mic icons are non-interactive placeholder chrome
...
### 39. [FIXED] `color` and `dotColor` in `MEAL_SLOT_CONFIG` are always identical
...
### 40. [FIXED] Accordion expand/collapse uses conditional rendering, not CSS animation
...
### 41. [FIXED] Array index in React key for food items
...
### 42. [OPEN] `capitalize` utility defined locally in multiple files
...
### 43. [FIXED] `getDisplayName`/`getFoodItems` not tested in isolation
...
### 44. [FIXED] SVG ring inlined in WaterModal (~45 lines)
...
### 45. [OPEN] Modal `if (!open) return null` prevents animated entry/exit
...
### 46. [OPEN] Focus trap queries DOM on every keydown
...
### 47. [OPEN] `MACRO_PILL_CONFIG` colors copy-pasted from NutritionCard
...
### 48. [OPEN] No entry/exit animation on LogFoodModal
...
### 49. [OPEN] `frequentFoods` alias mutable reference — implicit dependency identity
...
### 50. [OPEN] Tab panel `aria-label` computed with `TABS.find()` on every render
...
### 51. [OPEN] No `React.memo` or `useCallback` on Favourites/Filter row components
...
### 52. [FIXED] `FavouritesView` has no `useMemo` on `validFavourites`
...
### 53. [FIXED] `caloriesByMealSlot` iterates via four separate `calculateTotalCalories` calls
...
### 54. [FIXED] Redundant guard on accordion button click
...
### 55. [FIXED] `useNutritionGoals` spreads `profile.nutritionGoals` flat into return
...
### 56. [FIXED] `useMemo` wrapping return values in every `useProfile.ts` hook
...
### 57. [FIXED] `useHealthProfile` uses `as HealthProfile` cast on partial spread
...
### 58. [FIXED] Fuse.js no upper-bound query length guard
...
### 59. [FIXED] SVG `transition` inline style inconsistent with Tailwind approach
...
### 60. [FIXED] `recentFoods` fallback assignment outside all memos
...
### 61. [OPEN] Inline `rgba(249, 115, 22, 0.15)` bypasses design token
...
### 62. [OPEN] NutritionCard.tsx is ~805 lines with 8 co-located sub-components
...
### 63. [OPEN] No `React.memo` on any NutritionCard sub-component
...
### 64. [FIXED] `MealBreakdownBar` total not memoized
...
### 65. [FIXED] Time strings in `MEAL_SLOT_CONFIG` don't derive from `getMealSlot` boundaries
...
### 66. [OPEN] Magic number 50ms focus delay — `requestAnimationFrame` is more correct
...
### 67. [OPEN] LogFoodModal focus-restore 0ms `setTimeout` unnecessary
...
### 68. [OPEN] `FoodItemRow` and `MacroPill` could be extracted to own files

- **File**: `src/components/track/nutrition/LogFoodModal.tsx:82-161`
- **Agents**: 4
- **Suggestion**: Extract when file grows or tests added.

### 69. [OPEN] `formatPortion` pluralisation uses naive `+s` — fails on irregular plurals
...
### 70. [OPEN] Shared `portion + " . " + calories` format string duplicated in both row components

- **File**: `FavouritesView.tsx:118-119`, `FoodFilterView.tsx:253-255`
- **Agents**: 5
- **Suggestion**: Extract `formatPortionCalories()` utility.

### 71. [FIXED] No unit tests for FavouritesView or FoodFilterView
...
### 72. [OPEN] Store hook returns `state` as whole object — broad re-renders
...
### 73. [OPEN] `createInitialState` calls `getMealSlot(Date.now())` — flaky in tests
...
### 74. [OPEN] Module-level Fuse index constructed at import time
...
### 75. [FIXED] `headerIcons` JSX as render-body variable (non-idiomatic)
...
### 76. [OPEN] `onFocus` no-op callback passed to `NutritionSearchInput` from SearchView
...
### 77. [OPEN] Tab ARIA pattern incomplete — missing `tabindex` management and arrow-key nav
...
### 78. [FIXED] Hardcoded hex colors in CalorieDetailView config should use design tokens
...
### 79. [FIXED] `createStagedItem` uses `canonicalName` as `displayName` with no transformation
...
### 80. [FIXED] `generateStagingId` is a one-liner used once
...
### 81. [OPEN] `todayKey` number→string→number round-trip serves no purpose

- **File**: `src/hooks/useNutritionData.ts:94-101`
- **Agents**: 12
- **Suggestion**: Store as `number` directly, or remove `todayKey` entirely (see finding #3).

### 82. [FIXED] Early-exit condition in recent foods loop — misleading comment
...
### 83. [FIXED] SVG ring constants scattered across file header

- **File**: `src/components/track/nutrition/WaterModal.tsx:24-27`
- **Agents**: 9
- **Suggestion**: Cosmetic. Group into single object if desired.

### 84. [OPEN] `SearchResultRow` `FOOD_PORTION_DATA.get()` per render
