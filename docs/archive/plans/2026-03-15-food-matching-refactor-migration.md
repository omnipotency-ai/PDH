...
> **Archived on 2026-03-16:** the server-first food-matching migration in this plan is complete. Ongoing follow-up now lives in `docs/working/2026-03-15-food-registry-refactor.md`, `docs/working/2026-03-15-food-matching-refactor-session-handover.md`, and the backlog.

**Branch:** `refactor/food-matching`
...
1. Add shared phrase pre-processing and tests.
2. Add alias + embedding storage and server-side retrieval utilities.
3. Refactor `processLog` to run the new matcher and write richer item state.
4. Rework `FoodMatchingModal` and `resolveItem` around candidate confirmation / bucket selection / alias creation.
5. Downgrade the LLM path to explicit fallback-only usage and remove the current client auto-trigger.
6. Update tests around parsing, matching, modal flows, and evidence invariants.

## Completion Status — 2026-03-16

- [x] Shared deterministic phrase pre-processing landed with parsing tests.
- [x] Alias learning, embeddings, and server-side retrieval utilities landed.
- [x] `processLog` / server processing now own the main matching pipeline.
- [x] `FoodMatchingModal` was reworked into a server-fed resolution flow.
- [x] The LLM path is now fallback-only rather than the default unresolved route.
- [x] Parsing, matching, modal, evidence, and aggregate tests were updated around the new flow.

## Archived Outcome

This plan achieved its goal: the app no longer relies on the old `exact lookup -> client-triggered LLM -> manual picker` path as the primary flow. The remaining work is no longer migration work; it is taxonomy refinement, prompt/report alignment, and backlog cleanup on top of the shipped pipeline.
